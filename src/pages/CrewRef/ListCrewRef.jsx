...
const ListCrewRef = () => {
  ...
  const fetchCrewRef = () => {
    ...
    try {
      axios
        .get(
          `https://crew-backend-staging.vercel.app/api/crewref/all?page=${currentPage}`
        )
        .then((res) => {
          console.log(res);
          if (res.data.success) {
                        ...
                        {crewref?.map((item, idx) => (
                          <tr key={item._id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <button
                                onClick={() =>
                                  navigate(`/update-crewref/${item._id}`)
                                }
