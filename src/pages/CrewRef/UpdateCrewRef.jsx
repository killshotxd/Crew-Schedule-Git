...
const UpdateCrewRef = () => {
  ...
  const getCrewref = () => {
    ...
    try {
      axios
        .get(`https://crew-backend-staging.vercel.app/api/crewref/${params.id}`)
        .then((res) => {
          if (res.data.success) {
            const dataFlex = res.data.data;
            console.log(dataFlex);
            setFirstName(dataFlex.firstName);
            setLastName(dataFlex.lastName);
            setNickName(dataFlex.nickName);
            setLoading(false);
          }
        })
        .catch((error) => {
  ...
  const handleCrewRef = () => {
    ...
    try {
      axios
        .put(
          `https://crew-backend-staging.vercel.app/api/crewref/${params.id}`,
          data
        )
        .then((res) => {
          console.log(res);
          if (res.data.success) {
            setBtnDisable(false);
            toast.success("Crewref Updated Successfully !");
            setTimeout(() => {
